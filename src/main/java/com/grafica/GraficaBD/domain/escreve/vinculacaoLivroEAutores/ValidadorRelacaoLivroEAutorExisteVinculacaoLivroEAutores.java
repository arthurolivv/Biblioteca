...
@Component
public class ValidadorRelacaoLivroEAutorExisteVinculacaoLivroEAutores implements ValidadorVinculacaoLivroEAutores{
    ...
    public void validar(String isbn, AutoresPorRgDto autoresPorRgDto) {
        ...
        for(String autor_rg : autoresPorRgDto.autores_rg()){
            ...
            if(!jaExiste){
                throw new ResponseStatusException(HttpStatus.CONFLICT, "A relação entre o livro '" + isbn + "' e o autor '" + autor_rg + "' não existe!");
            }
        }
    }
}
